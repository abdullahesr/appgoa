  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setStats(prev => {
        ...
        if (isOptimized && rawFps > maxFpsLimit) { rawFps = maxFpsLimit + (Math.random() * 0.2); isFpsLimited = true; }
    ...
    return () => clearInterval(interval);
  }, [systemActive, selectedMode, isOptimized, isAutoTunerActive, t, playSound, maxFpsLimit, maxCpuLimit, maxGpuLimit, gpuOverclock, realLoad, advThermal, cooler, cpuSim, gpuSim, ramSim, unlockAchievement, addLog]);
    ...
    <div className={`min-h-screen p-4 flex flex-col items-center justify-start gap-4 max-w-md mx-auto relative overflow-hidden`}>
      ...
      <div className="w-full flex flex-col items-center gap-4">
          <div className="grid grid-cols-5 gap-1 bg-slate-900/50 p-1.5 rounded-full border border-slate-700 w-full">
            {fpsModeOptions.map(mode => ( <button key={mode} onClick={() => { playSound('click'); setSelectedMode(mode); }} className={`w-full py-2 rounded-full font-orbitron text-[10px] transition-all duration-300 ${ selectedMode === mode ? `${themes[theme].bg} text-slate-900 ${themes[theme].shadow}` : 'text-slate-400 hover:text-white' }`} > {mode === FpsMode.AUTO ? t.autoMode : `${mode} FPS`} </button> ))}
          </div>
      ...
      <main className="w-full flex flex-col items-center gap-4">
        ...
        <div className={`glass-panel p-5 rounded-2xl w-full ${isHighTemp ? 'animate-border-pulse' : ''}`}>
             ...
             <div className="space-y-4">
                <LimitSlider label={t.maxFpsLimit} value={maxFpsLimit} onChange={setMaxFpsLimit} min={30} max={240} unit="FPS" theme={theme} />
...
interface LimitSliderProps { label: string; value: number; onChange: (val: number) => void; min: number; max: number; unit: string; warning?: string; theme: Theme; }
const LimitSlider: React.FC<LimitSliderProps> = ({ label, value, onChange, min, max, unit, warning, theme }) => (
  <div className="flex flex-col gap-1.5">
    <div className="flex justify-between items-center px-1"> <span className="text-[9px] font-orbitron text-slate-500 uppercase">{label}</span> <span className={`text-[10px] font-orbitron ${themes[theme].text} font-bold`}>{value}{unit}</span> </div>
    <input type="range" min={min} max={max} step={1} value={value} onChange={(e) => onChange(parseInt(e.target.value))} className={`w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer ${themes[theme].accent}`} />
